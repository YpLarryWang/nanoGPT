...
class MaskedData:
    ...
    def mask_and_corrupt(self, buf, span_mask=False, generator=None):
        """buf: (B, L) original tokens. returns (corrupted_input, mask_flag)"""
        
        if span_mask:
            mask_flag = (self._span_mask_flag(buf.shape, generator=generator)) & (buf != self.eot_id)
        else:
            mask_flag = (torch.rand(buf.shape, generator=generator) < self.mask_p) & (buf != self.eot_id)
        r = torch.rand(buf.shape, generator=generator)
        corrupted = buf.clone()
        ...
        corrupted[rand_pos] = torch.randint(
            0, self.real_vocab, buf.shape, generator=generator
        )[rand_pos] # draw random tokens from the real vocab and selected idx in rand_pos
        return corrupted, mask_flag
        
    def _span_mask_flag(self, shape, max_span=3, generator=None):
        B, L = shape
        flag = torch.zeros(B, L, dtype=torch.bool)
        for b in range(B):                          # cheap: runs on CPU in the prefetch
            n, target = 0, int(self.mask_p * L)
            while n < target:
                s = torch.randint(1, max_span+1, (1,), generator=generator).item()
                i = torch.randint(0, L, (1,), generator=generator).item()
    ...
    def get_masked_batch(
        ...
        corrupted, mask_flag = self.mask_and_corrupt(buf, generator=corruption_generator)
