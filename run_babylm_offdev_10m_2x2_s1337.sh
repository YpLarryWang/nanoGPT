#!/usr/bin/env bash
# Smoke and then run the seed-1337 10M offdev depth x AttnRes 2x2 serially.
# Intended launch:
#   tmux new-session -d -s offdev10m-2x2-s1337 \
#     'cd /media/volume/yupei-data/repo/nanoGPT && bash run_babylm_offdev_10m_2x2_s1337.sh'
...
MIN_FREE_GB=220
QUEUE_NAME=offdev10m-2x2-s1337
LOG_DIR=logs/offdev-10m-2x2
...
arch_args() {
  ...
  ARCH_ARGS=(--n_layer="$n_layer" --n_embd=512 --n_head=8)
...
echo "schedule=$SCHEDULE smoke_root=$SMOKE_ROOT"
...
SCHEDULES=(
  config/checkpoint_schedules/bl10m-offdev-b8ga64-dual.json
...
expected_seeds = (1337, 1338, 1339)
payloads = [json.load(open(path, encoding="utf-8")) for path in sys.argv[1:]]
assert len(payloads) == 3
for payload, seed in zip(payloads, expected_seeds):
...
fingerprints = payloads[0]["fingerprints"]
actual = {
...
assert all(payload["fingerprints"] == fingerprints for payload in payloads)
assert actual == fingerprints, (actual, fingerprints)
print("validated three B8/GA64 schedules and all data fingerprints")
' "${SCHEDULES[@]}"

echo "==== phase 1: four 2-update smoke tests ===="
run_smoke "${NAMES[0]}" 32 0
run_smoke "${NAMES[1]}" 32 8
run_smoke "${NAMES[2]}" 16 0
run_smoke "${NAMES[3]}" 16 4

echo "==== phase 2: seed-1337 formal 2x2 ===="
run_formal "${NAMES[0]}" 32 0
run_formal "${NAMES[1]}" 32 8
run_formal "${NAMES[2]}" 16 0
run_formal "${NAMES[3]}" 16 4

printf '%s sha=%s log=%s\n' "$(date --iso-8601=seconds)" "$(git rev-parse HEAD)" "$QUEUE_LOG" \
  > "$DONE_MARKER"
echo "==== seed-1337 offdev 10M 2x2 queue complete ===="
