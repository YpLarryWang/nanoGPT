...
for name in ['blimp_filtered','supplement_filtered','entity_tracking','comps','global_piqa_parallel','global_piqa_nonparallel']:
 p=root/'evaluation_data/full_eval'/name
 assert p.is_dir() and any(p.rglob('*.json*')),(name,'missing')
print('Endpoint evaluation datasets downloaded',flush=True)
